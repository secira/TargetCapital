...
def save_user(user_claims):
    """Create or update user from Replit Auth claims - adapted for existing User model"""
    replit_user_id = user_claims['sub']
    
    # Try to find existing user by replit_id
    user = User.query.filter_by(replit_id=replit_user_id).first()
    
    if not user:
        # Create new user
        user = User()
        user.replit_id = replit_user_id
        user.username = user_claims.get('email', '').split('@')[0] if user_claims.get('email') else f'user_{replit_user_id}'
        user.email = user_claims.get('email')
        user.first_name = user_claims.get('first_name')
        user.last_name = user_claims.get('last_name')
        user.profile_image_url = user_claims.get('profile_image_url')
        user.is_verified = True  # OAuth users are verified by the provider
        db.session.add(user)
